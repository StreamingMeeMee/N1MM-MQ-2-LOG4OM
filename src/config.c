...
int config_load(const char *path, app_config_t *out_cfg, char *errbuf, size_t errbuf_size) {
    ...
    const char *rmq_reject_queue = json_get_string(rmq, "contactinfo_queue_reject", NULL);
    if (rmq_reject_queue && rmq_reject_queue[0]) {
        if (strcmp(rmq_reject_queue, rmq_queue) == 0) {
            snprintf(errbuf, errbuf_size,
                     "'rabbitmq.contactinfo_queue_reject' must differ from 'contactinfo_queue' "
                     "(rejected messages would be re-consumed forever)");
            json_free(root);
            return -1;
        }
        snprintf(out_cfg->rabbitmq.contactinfo_queue_reject, sizeof(out_cfg->rabbitmq.contactinfo_queue_reject),
                 "%s", rmq_reject_queue);
    }
